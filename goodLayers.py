def get_a_layer(keras, sample):
    ...
    leitura_mesa_layers = [
        keras.layers.MaxPooling2D(8 , 2,  # compress to aprox shape 100x100
                                   input_shape=(1025, 192, 1)),  # converts (shape) to (shape,1)
        keras.layers.BatchNormalization(),
        keras.layers.Flatten(),
        keras.layers.Dense(2**10, activation="softmax"),
        keras.layers.Dense(2**8, activation="softmax"),
        keras.layers.Dense(2**7, activation="softmax"),
        keras.layers.Dense(2**5, activation="softmax"),
